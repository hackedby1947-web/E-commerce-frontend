...
import React from 'react';
import { Trash2, Store, Heart, ShieldCheck, Minus, Plus, CheckCircle2, BadgeCheck } from 'lucide-react';
import { useCart } from '../context/CartContext';
import { useNavigate } from "react-router-dom";
...
export default function Cart() {
  ...
  const isAllSelected =
    cartItems?.filter(item => item.inStock).length > 0 &&
    cartItems?.filter(item => item.inStock).every(item => item.selected);
  ...
  const handleCheckout = () => {
    if (!cartItems || cartItems.length === 0) return;
    const selectedInStockItems = cartItems.filter(item => item.selected && item.inStock);
    if (selectedInStockItems.length === 0) {
      alert("No in-stock items selected for checkout.");
      return;
    }
    navigate("/checkout", { state: { items: selectedInStockItems } });
  };
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="px-2 mb-2">
            ...
            <p className="text-sm text-gray-500 font-bold">
              Total <span className="text-pink-600">{cartItems.length} items</span> added
            </p>
          </div>
          ...
          {cartItems.map((item) => {
            ...
            return (
              <div
                key={item._id}                              // ✅ fix
                className={`bg-white p-5 rounded-4xl transition-all border-2 ${
                  item.selected ? 'border-pink-500 shadow-md' : 'border-transparent shadow-sm'
                }`}
              >
                {/* Verified Shop Label */}
                <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-50">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      onClick={() => toggleSelect(item._id)}  // ✅ fix
                    >
                      {item.inStock ? (
                        item.selected ? (
                          <CheckCircle2 className="text-pink-600" size={22} />
                    ...
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#E7F3FF] border border-[#1877F2]/20 rounded-full shadow-sm">
                      <Store size={16} className="text-[#1877F2]" />
                      <p className="font-semibold text-[#1877F2] text-xs uppercase tracking-wider">Verified Shop</p>
                      <div className="flex items-center justify-center w-5 h-5 bg-[#1877F2] rounded-full">
                        <BadgeCheck size={13} className="text-white" />
                      </div>
                    </div>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <p className="text-2xl font-black text-pink-600">৳{item.price}</p>
                        {item.oldprice && (
                          <p className="text-xs text-gray-400 line-through font-bold">৳{item.oldprice}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-4">
                      ...
                      <div className="flex gap-2">
                        {item.colors?.map((color, i) => (
                          <button
                            key={i}
                            onClick={() => handleColorChange(item._id, color)}  // ✅ fix
                            className={`text-[10px] px-3 py-1 rounded-full font-bold transition-all border ${
                              (item.selectedColor || item.colors[0]) === color
                                ? 'bg-pink-600 text-white border-pink-600 shadow-md shadow-pink-100'
                                : 'bg-gray-50 text-gray-500 border-gray-200 hover:border-pink-300'
                    ...
                    <div className="flex items-center justify-between mt-6">
                      ...
                      <div className="flex gap-5 text-gray-400">
                        ...
                        <Trash2
                          ...
                          onClick={() => setCartItems(prev => prev.filter(i => i._id !== item._id))}  // ✅ fix
                        />
                      </div>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-50 sticky top-8">
            ...
            <button
              ...
              className={`w-full mt-10 py-5 rounded-2xl font-black text-white text-lg transition-all shadow-xl active:scale-95 ${
                selectedItems.length > 0 ? 'bg-black hover:bg-gray-800' : 'bg-gray-200 cursor-not-allowed text-gray-400'
              }`}
            >
              Checkout Now ({selectedItems.length})
            </button>
